...
def generate_pareto_curve(csv_file_path, output_csv_file_path, tableCount):
    ...
    plt.scatter(pareto_df['HowFilled'], pareto_df['AverageLookUp'], color='red', marker='o', label=f'Pareto Front for {tableCount} tables')
    plt.xlabel('HowFilled')
    plt.ylabel('AverageLookUp')
    plt.title(f'Pareto Curve for {tableCount} tables')
    plt.legend()
    plt.show()
...
def generate_plot(csv_filename, table_count):
    ...
    fig, ax = plt.subplots()
    ax.plot(X, Y, color='red', marker='o', label=f'Pareto Curve for {table_count} tables')
    ...
    plot_filename = os.path.join('Plots/Pareto', f'GenericHowFilled_{table_count}_Tables.png')
